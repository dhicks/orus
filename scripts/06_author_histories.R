...
scrape_ = function (this_auid, page_idx, max_results = 200, print_url = FALSE) {
    ## Basically just an abstraction of the RCurl call
    base_url = 'https://api.elsevier.com/content/search/scopus?'
    query_url = str_c(base_url, 
                      'query=au-id(', this_auid, ')', 'AND PUBYEAR BEF 2020',
                      '&count=', max_results,
                      '&start=', max_results*(page_idx-1),
...
scrape = function (this_auid, n_docs, target_folder, 
    ...
    total_pages = n_docs %/% max_results + 1
    ...
    for (page_idx in 1:total_pages) {
        ...
        if (!file.exists(target_file) | force_scrape) {
            raw = scrape_(this_auid, page_idx, max_results, ...)
...
# debugonce(parse_)
# file = str_c(author_folder, author_history_files[[3]])[2]
# file %>% 
#     read_file() %>% 
#     str_remove_all('<[0-9][:word:]+>') %>% 
#     parse_()
# parse_(read_file(file))

parse = function(files, author_folder) {
    files = str_c(author_folder, files)
    parsed_df = files %>% 
        map(read_file) %>% 
        ## Remove invalid xml tags introduced by encoding errors somewhere
        map(str_remove_all, '<[0-9][:word:]+>') %>% 
        map_dfr(parse_)
    return(parsed_df)
}
# debugonce(parse)
# parse(author_history_files[[1730]], author_folder)
...
## Because of the long gap between when we pulled the metadata and downloaded the paper lists, 
